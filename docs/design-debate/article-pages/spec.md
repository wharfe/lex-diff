...
**この保証の範囲（2026-09-22、Gate 2 周回3 で明確化）**: 守るのは
「**全法令を組み立てて検証し終えるまで、1バイトも書かない**」であって、
書き込みループそのものの原子性ではない。書き込みは法令ごとの
一時ファイル → `os.replace` なので、**ループの途中でプロセスが死ぬと、
先に置換された法令だけが新しくなる**。

ディレクトリごと入れ替える案は採らない: `articles` → `articles.old` →
`articles.new` → `articles` の2回リネームになり、その間に死ぬと
**ディレクトリごと消える**窓が生まれる。今の形の残余窓は「検証済みのファイルを
数個書くミリ秒」で、そこで死んでも残るのは**個別には妥当なファイル**である。
再実行で揃う。悪化させる取り替えはしない。
...
- `plain_summary` は対応する**履歴カード内へ移し**、時点を明示する。
  文言は改正の種類で分ける:
  - 通常の改正 → 「YYYY年M月D日**改正直後**の条文についての説明」
  - **削除**（`type == "deleted"`）→ 「YYYY年M月D日**改正直前**の条文についての説明」

  > **訂正（2026-09-22、Gate 2 周回3）**: 当初は一律「改正直後」としていたが、
  > 削除条文では事実と逆になる。民法754条の `paragraphs_after` は**空**で（実測）、
  > 改正直後の本文は範囲ノードの「削除」である。解説が述べているのは削除**前**の規定なので、
  > 「改正直後の条文についての説明」と付けると、廃止された規定の説明が
  > 「削除」という2文字の説明として読まれる。
...
### フィールドの出所（この表が正本。「今」と名乗る場所に過去の出所を使わない）

> **なぜ表にするか（2026-09-22、Gate 2 で2周連続の同根指摘）**: このページは
> **年齢の違う2つの出所**から組み立てられる。周回1 は「関連条文にゲートが掛かっていない」、
> 周回2 は「法令名が改名前のまま」を指摘した。**同じ根**であり、出てきた順に1つずつ直すと
> 次の1つが必ず残る。フィールドを閉じた表として列挙し、
> **「現在」と名乗るブロックに出るものは、今日の取得由来か、ゲートを通ったものだけ**にする。

| フィールド | 出所 | 条件 |
|---|---|---|
| `law_title` | **今日の取得**（`revision_info.law_title`） | 空なら失敗。改名が実在する（情プラ法） |
| `label`（漢数字の条名） | **今日の取得**（`ArticleTitle`） | — |
| `caption` | **今日の取得**（`ArticleCaption`、ルビ除去） | 取れなければ `<title>` から括弧ごと落とす |
| `section_path` | **今日の取得**（`diff.find_section_path`） | 編章の移動に追随する |
| `current.paragraphs` | **今日の取得** | 空なら失敗 |
| `article_num` / `slug` / `display_num` | 出荷 diff（識別子） | 今日の本文に無ければ `LookupError` で落ちる |
| `current_summary` | 出荷 diff | **版整合ゲートを通ったときだけ** |
| `related_articles` | 出荷 diff | **版整合ゲートを通ったときだけ** |
| `former` | 出荷 diff | **必ず日付付きで表示**（「今回の改正直前の条文」） |
| `changes[]` | 出荷 diff | **必ず施行日・改正法名と同じカード内**。`cross_references` はテキストのみ |

新しいフィールドを足すときは、この表に行を足してから実装する。
行が書けないフィールドは、ページに出さない。
...
広い範囲ノードへ番号が入るだけでは削除済みと判定しない。判定は**本文が「削除」であること**で行う。

> **訂正（2026-09-22、Gate 2 で発覚）**: R1 の批評がこの規則の根拠として挙げた
> 「育児介護休業法の `36:52` は17条ぶんの実体を覆う」は**事実に反する**。
> 実測すると今日の本文は「第三十六条から第五十二条まで削除」であり、
> `elm=Article_40` は 400 を返す（個別ノードは存在しない）＝これも欠番の範囲。
> 規則そのものは正しく倒れる（本文が「削除」なので `merged_deleted` になり、それが正解）が、
> **根拠として書いた例が偽**だった。実体のある広い範囲ノードが実在するという前提で
> この規則を「直す」ことのないように、訂正をここに残す。
